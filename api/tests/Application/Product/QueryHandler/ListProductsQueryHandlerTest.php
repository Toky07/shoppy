...
it('lists products newest first', function () {
    ...
    $response = (new ListProductsQueryHandler($repository))->handle(new ListProductsQuery());
...
it('paginates products', function () {
    ...
    $response = (new ListProductsQueryHandler($repository))->handle(new ListProductsQuery(page: 2, limit: 2));
...
it('rejects an invalid page', function () {
    (new ListProductsQueryHandler(new InMemoryProductRepository()))->handle(new ListProductsQuery(page: 0));
...
it('searches products by name and description', function () {
    ...
    $byName = (new ListProductsQueryHandler($repository))->handle(new ListProductsQuery(search: 'hoodie'));
    $byDescription = (new ListProductsQueryHandler($repository))->handle(new ListProductsQuery(search: 'logo'));
...
it('sorts products by price ascending', function () {
    ...
    $response = (new ListProductsQueryHandler($repository))->handle(new ListProductsQuery(sort: 'price_asc'));

    expect(array_column($response->toArray()['items'], 'name'))->toBe(['Mug', 'Tee', 'Hoodie']);
});

it('rejects an invalid sort', function () {
    (new ListProductsQueryHandler(new InMemoryProductRepository()))->handle(new ListProductsQuery(sort: 'popularity'));
})->throws(InvalidProductSort::class);

it('rejects a search that is too long', function () {
    (new ListProductsQueryHandler(new InMemoryProductRepository()))->handle(
